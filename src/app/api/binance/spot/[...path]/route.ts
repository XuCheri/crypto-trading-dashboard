...
export async function GET(
  ...
  try {
    ...
    const url = `${BINANCE_SPOT_API}${pathname}${searchParams ? `?${searchParams}` : ''}`

    console.log(`[Spot Proxy] Fetching: ${url}`)
    ...
    console.log(`[Spot Proxy] Success: ${pathname} (${duration}ms)`)
    ...
    console.error(`[Spot Proxy Error] ${errorMessage} (${duration}ms)`)
