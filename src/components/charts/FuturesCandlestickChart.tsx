...
interface FuturesCandlestickChartProps {
  symbol: string
...
export function FuturesCandlestickChart({
  ...
  const { data: klineData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return transformFuturesKlines(raw)
    },
    enabled: !!symbol,
    staleTime: 60000,
    retry: 2,
  ...
  // 根据市场类型使用不同的 WebSocket hook
  const useKlineStream = marketType === 'usdt-m' ? useFuturesKlineStream : useCoinMKlineStream
